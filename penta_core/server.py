...
An MCP server that aggregates the top 5 AI platforms into a single
"Swarm" toolset.
...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "- consult_architect: Systems architecture and design patterns "
        "(OpenAI GPT-4o)\n"
        "- consult_developer: Clean code, safety, and refactoring "
        "(Claude 3.5 Sonnet)\n"
        "- consult_researcher: Deep context analysis with massive context "
        "window (Google Gemini 1.5 Pro)\n"
        "- consult_maverick: Red teaming and unconventional solutions "
        "(xAI Grok Beta)\n"
        "- fetch_repo_context: Fetch file content/tree from GitHub "
        "repositories\n\n"
...
def _get_google_client() -> "google.generativeai":
...
def _get_xai_client() -> "openai.OpenAI":
    """Get xAI client using OpenAI SDK with custom base URL (lazy loading).

    Returns:
        OpenAI client instance configured for xAI API
...
def _get_github_headers() -> dict[str, str]:
    ...
    Returns:
        Dictionary containing GitHub API request headers
...
async def consult_researcher(prompt: str, context: str) -> str:
    ...
    try:
        ...
        full_prompt = (
            ...
            f"Context/Documentation to Analyze:\n{context}\n\n"
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 404:
                return (
                    f"Error: Repository or path not found: "
                    f"{owner}/{repo}/{path}"
                )
            ...
            if response.status_code != 200:
                return (
                    f"Error: GitHub API returned status "
            ...
            if isinstance(data, dict) and data.get("type") == "file":
                ...
                try:
                    ...
                    return (
                        f"File: {file_path} (binary content, "
                        f"{file_size} bytes)"
                    )
