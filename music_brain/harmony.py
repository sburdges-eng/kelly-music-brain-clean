...
class ChordVoicing:
    ...
    Attributes:
        root: Root note name (e.g., "F", "C", "Bbm")
        notes: MIDI note numbers (e.g., [60, 64, 67] for C major)
        duration_beats: Duration in beats (quarter note = 1 beat). Defaults to 4.0
        velocity: MIDI velocity (0-127). Defaults to 80
        roman_numeral: Optional Roman numeral notation (e.g., "I", "V", "vi")
        emotional_function: Optional description of emotional function
    """
    root: str
    notes: List[int]
...
class HarmonyResult:
    ...
    Attributes:
        ...
        voicings: List of ChordVoicing objects with full MIDI voicing data
        key: Musical key (e.g., "F", "C")
        mode: Mode (e.g., "major", "minor")
        rule_break_applied: Optional name of rule-breaking technique applied
        emotional_justification: Optional explanation of why rule was broken
    """
    chords: List[str]
...
class HarmonyGenerator:
    ...
    def generate_basic_progression(
        self, key: str, mode: str = "major", pattern: str = "I-V-vi-IV"
    ) -> HarmonyResult:
        """
        ...
        return HarmonyResult(
            chords=chord_symbols, voicings=voicings, key=key, mode=mode
        )
    ...
    def _roman_to_chord_symbols(
        self, roman_numerals: List[str], key: str, mode: str
    ) -> List[str]:
        """
        ...
        """
        scale = (
            self.SCALES["major"] if mode == "major" else self.SCALES["natural_minor"]
        )
        ...
        chord_map_minor = {
            ...
            "VI": (5, "maj"),
            "VII": (6, "maj"),
            "V": (4, "maj"),  # V can be major in minor keys
        }
    ...
    def _chords_to_voicings(
        self, chord_symbols: List[str], key: str
    ) -> List[ChordVoicing]:
        """
    ...
    def _apply_modal_interchange(
        self, base_progression: List[str], key: str, mode: str, intent: "CompleteSongIntent"
    ) -> List[str]:
        """
...
def generate_midi_from_harmony(
    harmony: HarmonyResult, output_path: str, tempo_bpm: int = 82
):
    """
    ...
    for voicing in harmony.voicings:
        ...
        for note in voicing.notes:
            track.append(
                Message("note_on", note=note, velocity=voicing.velocity, time=0)
            )
...
if __name__ == "__main__":
    ...
    generate_midi_from_harmony(
        kelly_harmony, "/home/claude/kelly_song_harmony.mid", tempo_bpm=82
    )
    ...
    basic_harmony = generator.generate_basic_progression(
        key="C", mode="major", pattern="I-V-vi-IV"
    )

    generate_midi_from_harmony(
        basic_harmony, "/home/claude/basic_progression.mid", tempo_bpm=120
    )
