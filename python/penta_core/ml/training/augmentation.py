...
class AudioAugmentor:
    ...
    def augment_batch(self, audio_batch: torch.Tensor, sr: int = 44100) -> torch.Tensor:
        """
        ...
        audio_batch = audio_batch * gain
        
        # 2. Add white noise
        if random.random() < self.config.p_noise:
            noise = torch.randn_like(audio_batch) * 0.01
            audio_batch = audio_batch + noise
    ...
    def harmonic_mix(
        ...
        """
        Mixes two waveforms only if they are harmonically compatible.
        In this implementation, we ensure length and phase alignment.
        """
        alpha = alpha or self.config.harmonic_mix_alpha
        
        # Ensure equal length
        min_len = min(len(y1), len(y2))
        y1, y2 = y1[:min_len], y2[:min_len]
        
        # Phase alignment (simple cross-correlation shift)
        if random.random() < self.config.p_phase_align:
            y2 = self.align_phase(y1, y2)
            
        return (1 - alpha) * y1 + alpha * y2

    def align_phase(self, y1: np.ndarray, y2: np.ndarray) -> np.ndarray:
        """
        Aligns the phase of y2 to y1 using cross-correlation.
        Prevents comb filtering when mixing similar sources (e.g. two kicks).
        """
