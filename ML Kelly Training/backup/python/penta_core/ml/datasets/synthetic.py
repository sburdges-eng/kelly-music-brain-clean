...
class SyntheticGenerator:
    ...
    def _parse_chord_root(self, chord_symbol: str, scale: List[int]) -> int:
        ...
        # Remove inversion/alternate bass (e.g., V/vi)
        chord_root = chord_symbol.split('/')[0]

        # Handle flat prefix (bII, bVI, etc.)
        flat_offset = 0
        if chord_root.startswith(('b', 'B')):
            flat_offset = -1
            chord_root = chord_root[1:]

        # Extract base numeral
        base = ''
        for c in chord_root.upper():
            if c in 'IVX':
                base += c
            else:
                break
        ...
        if base in numeral_map:
            scale_degree = numeral_map[base] + flat_offset
            if scale_degree < 0:
                scale_degree = (scale_degree + len(scale)) % len(scale)
